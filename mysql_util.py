...
class MysqlUtil():
    ...
    def __init__(self):
        ...
        self.db = pymysql.connect(host=host,
                                  ...
                                  password=password,
                                  db=database,
                                  charset=charset)  # 建立连接
        self.cursor = self.db.cursor(
            cursor=pymysql.cursors.DictCursor)  # 设置游标，并将游标设置为字典类型,结果作为字典返回
        # self.cursor = self.db.cursor()  # 设置游标，并将游标设置为字典类型
    ...
    def fetchone(self, sql):
        ...
        try:
            ...
            # 输出异常信息:ex_type, ex_val, ex_stack
    ...
    def delete(self, sql):
        ...
        try:
            ...
            f.flush()  # 调用文件写方法时，紧接着调用flush（）方法，可以将文件立即写入磁盘中
            f.close()
            # 如果发生异常，则回滚
            self.db.rollback()
        finally:
